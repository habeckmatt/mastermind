...
# This method shows the mode options and makes sure that the mode
# that the user chooses is valid.
    def choose_mode
        valid = false
        while valid == false
            puts "Hi #{@human_player.name} do you want to be the code maker \n or the code
            breaker?"
...
# or if the iteration runs out.
    ...
    def play_as_codemaker
        ...
        while i < 11
            if i == 1
                @guess = @computer_player.initial_guess
            else
...
# This method makes the computers intial guess, 4 random letters from the letters array
        ...
        def initial_guess
            while @initial_guess.size < 4
                @initial_guess << LETTERS[rand(LETTERS.size)]
                @initial_guess.uniq!
            end
